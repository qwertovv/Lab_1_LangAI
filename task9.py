...
if __name__ == "__main__":
    ...
    # Дополнительный пример: демонстрация бесконечной природы генератора
    print("\n" + "=" * 60)
    print("Дополнительная демонстрация:")
    print("=" * 60)
    
    # Создаем новый генератор
    print("\nСоздаем новый генератор паролей...")
    my_gen = password_generator(6)  # короткие пароли для демонстрации
    
    # Используем генератор как итератор
    print("\nПервые 10 паролей из нового генератора:")
    for i in range(10):
        print(f"  Пароль {i+1}: {next(my_gen)}")
    
    # Покажем, что можно продолжать бесконечно
    ...
    print("\n" + "=" * 60)
    print("Задача выполнена! Генератор может работать бесконечно.")
    print("=" * 60)